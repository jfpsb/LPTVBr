<div class="album-panel">
	<img src="/LPTVBr/resources/imagens/albuns/ats250.jpg"
		class="album-cover"> <a class="album-link"
		href="/LPTVBr/musicas/albuns/a-thousand-suns/">Voltar ao A Thousand Suns</a>
	<ol class="album-lista">
		<li class="traducao-li"><a class="album-text"
			href="the-requiem">1. The
				Requiem</a></li>
		<li class="traducao-li"><a class="album-text"
			href="the-radiance">2. The
				Radiance</a></li>
		<li class="traducao-li"><a class="album-text"
			href="burning-in-the-skies">3.
				Burning In The Skies</a></li>
		<li class="traducao-li"><a class="album-text"
			href="empty-spaces">4.
				Empty Spaces</a></li>
		<li class="traducao-li"><a class="album-text"
			href="when-they-come-for-me">5.
				When They Come For Me</a></li>
		<li class="traducao-li"><a class="album-text"
			href="robot-boy">6. Robot Boy</a></li>
		<li class="traducao-li"><a class="album-text"
			href="jornada-del-muerto">7.
				Jornada Del Muerto</a></li>
		<li class="traducao-li"><a class="album-text"
			href="waiting-for-the-end">8.
				Waiting For The End</a></li>
		<li class="traducao-li"><a class="album-text"
			href="blackout">9. Black Out</a></li>
		<li class="traducao-li"><a class="album-text"
			href="wretches-and-kings">10.
				Wretches And Kings</a></li>
		<li class="traducao-li"><a class="album-text"
			href="wisdow-justice-and-love">11.
				Wisdow, Justice And Love</a></li>
		<li class="traducao-li"><a class="album-text"
			href="iridescent">12.
				Iridescent</a></li>
		<li class="traducao-li"><a class="album-text"
			href="fallout">13. Fallout</a></li>
		<li class="traducao-li"><a class="album-text"
			href="the-catalyst">14. The
				Catalyst</a></li>
		<li class="traducao-li"><a class="album-text"
			href="the-messenger">15.
				The Messenger</a></li>
	</ol>
	<p class="album-copyright" align="center">
	Linkin Park Sep 14, 2010 Ⓟ Warner Bros. Records Inc.
	</p>
</div>