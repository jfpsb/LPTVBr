<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<link rel="shortcut icon" href="/LPTVBr/favicon.ico" type="image/x-icon" />
<link id="favicon" href="/LPTVBr/favicon.ico" rel="shortcut icon"
	type="image/vnd.microsoft.icon"></link>
<link rel="shortcut icon" href="/LPTVBr/favicon.ico" type="image/x-icon" />
<link id="favicon" href="/LPTVBr/favicon.ico" rel="shortcut icon"
	type="image/vnd.microsoft.icon"></link>
<script type="text/javascript"
	src="/LPTVBr/resources/js/jquery-1.11.3.min.js"></script>
<link rel="stylesheet" type="text/css"
	href="/LPTVBr/resources/css/mus-lista.css">
<link rel="stylesheet" type="text/css"
	href="/LPTVBr/resources/css/mus-lista-bg.css">
<link rel="stylesheet" type="text/css"
	href="/LPTVBr/resources/css/global.css">
<script type="text/javascript"
	src="/LPTVBr/resources/js/focusMainSection.js"></script>
<title>Remixes : A Light That Never Comes</title>
</head>
<body>
	<?php
	include_once '../../../template/header.php';
	?>
	
 	<section id="main-section" class="body-section">
		<article id="altncr-article" class="table-article">
			<div class="table">
				<div class="panel-header">
					<h2 class="header">A Light That Never Comes</h2>
				</div>
				<table class="mus-table">
					<tr>
						<td>1.</td>
						<td>A Light That Never Comes (Rick Rubin Reboot)</td>
						<td id="button-td"><a class="trad-button"
							href="altnc-rick.php">Tradução</a></td>
					</tr>
					<tr>
						<td>2.</td>
						<td>A Light That Never Comes (Vicetone Remix)</td>
						<td id="button-td"><a href="altnc-vicetone.php"
							class="trad-button">Tradução</a></td>
					</tr>
					<tr>
						<td>3.</td>
						<td>A Light That Never Comes (Angger Dimas Remix)</td>
						<td id="button-td"><a href="altnc-angger.php"
							class="trad-button">Tradução</a></td>
					</tr>
					<tr>
						<td>4.</td>
						<td>A Light That Never Comes (Twoloud Remix)</td>
						<td id="button-td"><a
							href="altnc-twoloud.php" class="trad-button">Tradução</a></td>
					</tr>
					<tr>
						<td>5.</td>
						<td>A Light That Never Comes (Coone Remix)</td>
						<td id="button-td"><a
							href="altnc-coone.php"
							class="trad-button">Tradução</a></td>
					</tr>
					<tr>
						<td>6.</td>
						<td>A Light That Never Comes (Vicetone Remix Dub)</td>
						<td id="button-td"><a
							href="altnc-vicetone-dub.php"
							class="trad-button">Tradução</a></td>
					</tr>
					<tr>
						<td>7.</td>
						<td>A Light That Never Comes (Brian Yates Remix)</td>
						<td id="button-td"><a
							href="altnc-brian.php"
							class="trad-button">Tradução</a></td>
					</tr>
				</table>
			</div>
		</article>
	</section>
	
	<?php include_once '../../../template/footer.php';?>
</body>
</html>